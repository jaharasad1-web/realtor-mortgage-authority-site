...
function html(message,status=200){return new Response(`<!doctype html><html><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Contact Jahar Asad</title><style>body{font-family:Arial,sans-serif;background:#EEF5FA;color:#2B2B2B;margin:0;padding:40px}.card{max-width:680px;margin:60px auto;background:white;padding:32px;border-radius:18px;box-shadow:0 10px 30px rgba(10,22,51,.10)}h1{color:#0A1633}a{color:#0A1633;font-weight:700}</style></head><body><div class="card"><h1>Contact Jahar</h1><p>${message}</p><p><a href="/contact.html">Return to Contact</a> · <a href="/">Return Home</a></p></div></body></html>`,{status,headers:{"content-type":"text/html; charset=utf-8","cache-control":"no-store"}})}
...
export async function onRequestPost({request,env}){
  ...
  body.page_url=body.page_url||new URL(request.url).origin+"/contact.html";
  ...
  if(response.ok&&data.ok)return Response.redirect(new URL("/thank-you.html",request.url),303);
