const ALLOWED_CAMPAIGNS = {
  ...
  "reverse-mortgage": {
    ...
    contactType: "mortgage",
    source: "Website | Facebook | Reverse Mortgage / HECM"
  }
};
...
async function supabaseFetch(env, path, init = {}) {
  ...
  // New Supabase sb_secret_... keys are server-side API keys.
  // Send them in apikey only; do NOT send as Authorization: Bearer.
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    if (existing?.id) {
      ...
      if (!familyId) {
        throw new Error("CRM record created but no family ID was returned.");
      }
    }
